...
TEST(symbolic, modsolver_sub_wrap_multiple_of_m) {
  vkcnn::SymGraph g;
  auto A = g.createParameter();
  // (A - 32) % 8 == A % 8  (structure-equality, not constant)
  auto lhs = g.mod(g.sub(A, 32), 8);
  auto rhs = g.mod(A, 8);
  EXPECT_EQ(rhs, lhs);
}
...
TEST(symbolic, modsolver_nested_mod_rhs_collapses_via_solver) {
  vkcnn::SymGraph g;
  auto X = g.createParameter(), Y = g.createParameter();
  // inner: (4*Y + 1) % 4 = 1
  // X % ( (4Y+1)%4 )  => X % 1  => 0
...
TEST(symbolic, modsolve_evenizer_general_k_equal_modulus) {
  vkcnn::SymGraph g; auto E = g.createParameter();
  std::size_t k = 12;
  // (E + (k - (E % k))) % k == 0

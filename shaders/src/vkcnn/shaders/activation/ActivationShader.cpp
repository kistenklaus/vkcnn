...
static std::vector<std::byte> source() {
  std::string srcStr =
      vkcnn::readFile("./shaders/src/vkcnn/shaders/activation/activation.comp");
  std::string str = shaders::preprocess_shader_src_pragmas(srcStr);

  std::vector<std::byte> src(str.size() * sizeof(std::string::value_type));
  std::memcpy(src.data(), str.data(),
              str.size() * sizeof(std::string::value_type));
  return src;
}

ActivationShader::ActivationShader(glm::uvec3 iTile, glm::uvec3 wTile)
    : m_tileSizes(TileSizes(iTile, wTile)), m_source(source()),
      m_name(fmt::format("activation_W{}x{}x{}_I{}x{}x{}", wTile.x, wTile.y,
                         wTile.z, iTile.x, iTile.y, iTile.z)) {}

ActivationShader::ActivationShader()
    : m_tileSizes(std::nullopt), m_source(source()), m_name("activation") {}

bool ActivationShader::supports(const OpActivation &op) const {
  ...
  if (op.inputLayout == ActivationLayout::CHWC4 ||
...
ActivationShader::do_specialize(const OpActivation &op) const {
  ...
  if (op.function == ActivationFunction::ReLU) {
  ...
  return ActivationShaderSource(
      m_source, ShaderLang::GLSL, SpecializationConstants{},
      ShaderDefines{defines}, tileSize,
      ActivationShaderSource::DebugInfo(op.inputLayout, op.inputType,
                                        op.outputLayout, op.outputType,
                                        op.channels, op.function, m_name));
}

std::string_view ActivationShader::name() const { return m_name; }
